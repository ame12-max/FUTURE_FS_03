  ...
  return (
    <div className="min-h-screen pt-28 pb-20 bg-black/80">
      <div className="container mx-auto px-6">
        ...
        >
          <h1 className="text-3xl font-playfair font-bold text-gold mb-2">{t('dashboard.title')}</h1>
          <p className="text-gray-300 mb-8">{t('dashboard.welcome')} {user?.name}</p>
          
          <div className="bg-white/5 backdrop-blur-sm rounded-2xl p-6 border border-white/10">
            <h2 className="text-xl font-playfair font-bold text-gold mb-4 flex items-center gap-2">
              <FiShoppingBag /> {t('dashboard.orderHistory')}
            </h2>
            
            {orders.length === 0 ? (
              <div className="text-center py-8">
                <p className="text-gray-400 mb-4">{t('dashboard.noOrders')}</p>
                ...
                {orders.map((order) => (
                  ...
                  >
                    <div className="flex justify-between items-center flex-wrap gap-3">
                      <div>
                        <p className="text-gold font-semibold">{t('dashboard.order')} #{order.id}</p>
                        <p className="text-sm text-gray-400">
                          {new Date(order.created_at).toLocaleDateString()} at {new Date(order.created_at).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                        </p>
                        <p className="text-sm text-gray-300 mt-1">{t('dashboard.total')}: {getSymbol()}{convertPrice(order.total_amount)}</p>
                      </div>
                      <div>
