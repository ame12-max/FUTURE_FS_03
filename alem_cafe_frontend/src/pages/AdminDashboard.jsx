...
const AdminDashboard = () => {
  ...
  const stats = [
    { 
      ...
      value: analytics?.total?.total || 0, 
      icon: FiShoppingBag, 
      color: 'text-blue-400' 
    },
    { 
      title: t('adminDashboard.revenue'), 
      value: `${getSymbol()}${analytics?.total?.revenue || 0}`, 
      icon: FiDollarSign, 
      color: 'text-green-400' 
    },
    { 
      title: t('adminDashboard.todayOrders'), 
      value: analytics?.today?.todayOrders || 0, 
      icon: FiTrendingUp, 
    ...
    <div className="min-h-screen pt-24 pb-12 bg-black/50">
      <div className="container mx-auto px-6">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
          <h1 className="text-3xl font-playfair font-bold text-gold mb-2">{t('adminDashboard.title')}</h1>
          <p className="text-gray-300 mb-8">{t('adminDashboard.welcome')} {user?.name}</p>
          ...
          <div className="grid md:grid-cols-2 gap-6">
            <div className="bg-white/5 backdrop-blur-sm rounded-2xl p-6 border border-white/10">
              <h2 className="text-xl font-bold text-gold mb-4">{t('adminDashboard.ordersByStatus')}</h2>
              {analytics?.byStatus?.map(status => (
                <div key={status.status} className="flex justify-between py-2 border-b border-white/10">
                  <span className="capitalize text-gray-300">{t(`status.${status.status}`, status.status)}</span>
            ...
            <div className="bg-white/5 backdrop-blur-sm rounded-2xl p-6 border border-white/10">
              ...
              <div className="space-y-3">
                ...
                <Link to="/admin/reservations" className="block text-center bg-gold/20 hover:bg-gold/30 text-gold py-2 rounded-lg transition">
                  {t('adminDashboard.manageReservations')}
                </Link>
              </div>
            </div>
          </div>
